...
export function OperatorApp() {
  ...
  /** "Devam Et" — dismiss warning and restart the idle timer. */
  ...
  // ─── Activity listeners — only mount/unmount when operator changes ────────
  ...
  useEffect(() => {
    ...
    // Intentionally ONLY re-run on operator change.
    // resetIdleTimer / startCountdown are stable (no showWarning dep).
  ...
  useEffect(() => {
    const boot = async () => {
      ...
      if (!raw) { setBootLoading(false); return; }
