...
import { Plus, Package, Search, Filter, Box, BarChart2, ChevronDown, ChevronUp, RotateCcw, Scissors } from 'lucide-react';
...
interface DiesPageProps {
  onNavigateToDashboard: () => void;
  onNavigateToPreMachining: () => void;
}
...
export function DiesPage({ onNavigateToDashboard, onNavigateToPreMachining }: DiesPageProps) {
  // ── list state ──────────────────────────────────────────────────────────
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <button
            onClick={onNavigateToPreMachining}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-2">
        <div className="flex flex-wrap gap-3 p-4">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${showAdvanced || advancedActive
...
function DieCard({ die, selected, onSelect, onCreateProductionOrder }: DieCardProps) {
  return (
    <div
      onClick={onSelect}
      className={`bg-white rounded-lg shadow-sm border-2 p-4 cursor-pointer transition-all ${selected ? 'border-blue-500 shadow-md' : 'border-gray-200 hover:border-gray-300'
        }`}
    >
